...
frm_card = 0 # тут буде пов'язуватися питання з формою тесту
timer = QTimer() # таймер для можливості "заснути" на певний час і прокинутися
...
def click_OK():
    ...
    if btn_OK.text() != 'Наступне питання':
        ...
        # напис на кнопці дорівнює 'Наступне', от і створюємо наступне випадкове питання:
        show_random()
...
def set_main():
    ...
    win_main.setWindowIcon(QIcon('brain-ok3.png'))

def edit_question(index):
    ''' завантажує у форму редагування питання і відповіді, що відповідають переданому рядку '''
...
def add_form():
    ...
    last = questions_listmodel.rowCount(0) - 1   # Нове питання - останнє у списку. Знайдемо його позицію. 
                                                ...
                                                # потрібен за стандартом бібліотеки (метод з параметром index викликається під час відображення списку)    
    ...
    edit_question(index)    # це питання потрібно завантажити у форму редагування
                            # кліка мишкою у нас тут немає, викличемо потрібну функцію з програми
    txt_Question.setFocus(Qt.TabFocusReason) # переводимо фокус на поле редагування питання, щоб відразу прибиралися "заготовки"
                                             # Qt.TabFocusReason переводить фокус так, як якби була натиснута клавіша "tab" 
                                             # це зручно тим, що виділяє "заготовку", її легко відразу прибрати 
...
def start_test():
    ''' на початку тесту форма пов'язується з випадковим питанням і показується'''
    show_random()
    win_card.show()
...
def connects():
    list_questions.setModel(questions_listmodel) # пов'язати список на екрані зі списком питань
    ...
    btn_add.clicked.connect(add_form) # поєднали натискання кнопки "нове питання" з функцією додавання
    btn_delete.clicked.connect(del_form) # поєднали натискання кнопки "видалити питання" з функцією видалення
    ...
    btn_Menu.clicked.connect(back_to_menu) # натискання кнопки "Меню" для повернення з форми тесту у редактор питань
